...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['add_product'])) {
    ...
    } elseif (preg_match('/^(.)\1+$/', preg_replace('/\s+/', '', strtolower($name)))) {
        $invalidFields[] = "name";
        $errors[] = "Product name must be meaningful.";
    }
        ...
        if (!in_array($fileType, $allowedTypes, true) || !in_array($extension, $allowedExtensions, true)) {
            ...
            if (!move_uploaded_file($_FILES['image']['tmp_name'], $targetPath)) {
                ...
                if ($insert) {
                    ...
                    if (function_exists('signData') && function_exists('storeSignatureRecord')) {
    ...
    <div class="container">
        ...
        <?php else: ?>
            ...
            <div class="sell-form-card">
                ...
                <?php if ($error): ?>
                    <div class="error-msg"><?php echo nl2br(h($error)); ?></div>
                <?php endif; ?>
...
if (sellForm) {
    sellForm.addEventListener("submit", function(e) {
        ...
        if (!name.value.trim() || !/[a-zA-Z]/.test(name.value) || /^(.)\1+$/.test(cleanName)) {
            markInvalid(name);
        }
