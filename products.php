...
if ($search !== "") {
    $products = tsRankProductsForSearch($search, $products);
}
?>
<!DOCTYPE html>
<html lang="en">
...
<div class="page-wrap">
    <div class="container">
        ...
        <p class="section-subtitle">
            Search products by name, category, description, city, or condition. Related items are ranked using TF-IDF and cosine similarity.
        </p>
                ...
                data.forEach(item => {
                    html += `
                        <a href="products.php?search=${encodeURIComponent(item.name)}" class="suggestion-item">
                            ...
                            <small>${escapeHtml(item.category ? item.category : "Suggested result")}</small>
                        </a>
                    `;
                });
