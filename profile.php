...
$completedSalesRes = $conn->query("SELECT COUNT(*) AS total FROM orders WHERE seller_user_id=$userId AND buyer_received=1");
$completedSales = $completedSalesRes ? (int)$completedSalesRes->fetch_assoc()['total'] : 0;

$receivedOrdersCountRes = $conn->query("SELECT COUNT(*) AS total FROM orders WHERE seller_user_id=$userId");
...
<head>
    ...
    <style>
        ...
        .order-action-popup.success-green{background:#059669;box-shadow:0 10px 25px rgba(5,150,105,.25);}
        ...
        .profile-show-all-row{text-align:center;margin-top:22px;}
        .profile-show-all-row .small-btn{display:inline-flex;align-items:center;justify-content:center;min-width:170px;}
        ...
        .rating-modal{width:100%;max-width:430px;background:white;border-radius:22px;padding:28px;position:relative;box-shadow:0 25px 60px rgba(0,0,0,.28);animation:modalPop .25s ease;}
        ...
        @media(max-width:992px){.profile-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important;}}
    ...
    <?php if ($myWishlist && $myWishlist->num_rows > 0): ?>
            ...
            <?php while ($row = $myWishlist->fetch_assoc()): ?>
                <div class="product-card wishlist-card" id="wishlist-card-<?php echo (int)$row['id']; ?>">
                    <div class="product-image-wrap">
                        ...
                        <?php if ($row['status'] === 'sold'): ?><div class="sold-badge">SOLD</div><?php endif; ?>
                    </div>
        ...
        <?php if ($wishlistCount > 6): ?>
            <div class="profile-show-all-row">
                <a href="my_wishlist.php" class="small-btn dark">Show All Wishlist</a>
            </div>
        <?php endif; ?>
    <?php else: ?>
...
<div class="profile-section-card" id="purchases">
    ...
    <?php if ($myPurchases && $myPurchases->num_rows > 0): ?>
            ...
            <?php while ($row = $myPurchases->fetch_assoc()): ?>
                <div class="product-card tracked-order-card <?php echo ($highlightOrderId === (int)$row['id']) ? 'active-track' : ''; ?>" id="order-card-<?php echo (int)$row['id']; ?>">
                    <?php if ($highlightOrderId === (int)$row['id']): ?>
                        <div class="order-action-popup success-green"><?php echo htmlspecialchars($highlightMessage); ?></div>
                    <?php endif; ?>
        ...
        <?php if ($totalPurchases > 6): ?>
            <div class="profile-show-all-row">
                <a href="my_purchases.php" class="small-btn dark">Show All Purchases</a>
            </div>
        <?php endif; ?>
    <?php else: ?>
...
<div class="profile-section-card" id="orders_received">
    ...
    <?php if ($receivedOrders && $receivedOrders->num_rows > 0): ?>
                    ...
                    <div class="product-body">
                        ...
                        <div class="product-actions" style="display:flex;flex-direction:column;gap:10px;align-items:stretch;margin-top:12px;">
                            ...
                            <form method="POST" style="margin:0;">
                                ...
                                <button type="submit" name="update_delivery_status" class="small-btn dark" style="width:100%;">Update Delivery Status</button>
                            </form>
                        </div>
                    </div>
                </div>
        ...
        <?php if ($receivedOrdersCount > 6): ?>
            <div class="profile-show-all-row">
                <a href="received_orders.php" class="small-btn dark">Show All Received Orders</a>
            </div>
        <?php endif; ?>
    <?php else: ?>
...
<div class="profile-section-card" id="sales">
    ...
    <?php if ($mySales && $mySales->num_rows > 0): ?>
        ...
        <?php if ($completedSales > 6): ?>
            <div class="profile-show-all-row">
                <a href="completed_sales.php" class="small-btn dark">Show All Completed Sales</a>
            </div>
        <?php endif; ?>
    <?php else: ?>
